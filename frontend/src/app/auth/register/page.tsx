      ...
      <div className="flex-1 flex items-center justify-center p-4 lg:p-8">
        ...
        >
          ...
          <div className="text-center mb-6">
            <h1 className="text-xl font-bold text-charcoal mb-1.5">Create Account</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="space-y-1 p-2.5 bg-charcoal-50 rounded-lg">
                {passwordRequirements.map((req, index) => (
                  <div key={index} className="flex items-center gap-2 text-[11px]">)
                    ...
                    <span className={req.test(formData.password) ? 'text-green-600' : 'text-charcoal-400'}>
                      {req.label}
                    </span>
                  </div>
                ))}
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input 
                ...
                className="mt-0.5 rounded border-charcoal-200 text-pink-500 focus:ring-pink-400" 
              />
              <span className="text-[11px] text-charcoal-500">
                I agree to the{' '}
                <Link href="/terms" className="text-pink-500 hover:text-pink-600">Terms of Service</Link>
                {' '}and{' '}
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="flex items-center justify-center gap-2 px-4 py-2.5 border border-charcoal-200 rounded-xl hover:bg-soft-pink transition-colors">
              ...
              <span className="text-xs font-medium text-charcoal-700">Google</span>
            </button>
            <button className="flex items-center justify-center gap-2 px-4 py-2.5 border border-charcoal-200 rounded-xl hover:bg-soft-pink transition-colors">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="#1877F2">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
              <span className="text-xs font-medium text-charcoal-700">Facebook</span>
            </button>
          </div>
